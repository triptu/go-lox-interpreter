...
// As WASm runs on the main thread it bogs down the UI, so we run it in a worker
...
async function initWasm() {
	...
	// @ts-expect-error - we're using the global go object
	...
	async (e) => {
		try {
			switch (e.data.type) {
				case "run":
					await initWasm();
					self.loxrun?.("run", e.data.code, (event) => {
						postMessage(event);
					});
					break;
				case "stop":
			...
			console.error(err);
			postMessage({ type: "error", data: `Webworker error: ${err}` });
		}
	},
	false,
);
